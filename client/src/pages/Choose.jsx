  ...
  return (
    <div className="min-h-screen mesh-bg">
      ...
      <div className="min-h-screen flex flex-col items-center justify-center section-container py-24">
        ...
        >
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-6" style={{ background: 'linear-gradient(135deg, #C76D4A, #8A9A7B)' }}>
          ...
          {options.map(({ id, icon: Icon, title, description, href, gradient, glow, features, cta }, i) => (
            ...
            >
              <Link to={href} className="group block">
                <div className="glass-card p-8 h-full transition-all duration-300 group-hover:shadow-[0_12px_40px_rgba(91,70,54,0.15)] group-hover:-translate-y-2 relative overflow-hidden">
                  {/* Hover glow */}
                  ...
                  <div className={`inline-flex items-center gap-2 text-sm font-semibold bg-gradient-to-r ${gradient} bg-clip-text text-transparent group-hover:gap-3 transition-all duration-200`}>
                    {cta} <ArrowRight className={`w-4 h-4 bg-gradient-to-r ${gradient} [&>path]:stroke-current`} style={{ color: id === 'customize' ? '#C76D4A' : '#8A9A7B' }} />
                  </div>
                </div>
              </Link>
            </motion.div>
